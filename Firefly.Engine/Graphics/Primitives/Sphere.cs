...
namespace Firefly.Engine.Graphics.Primitives
{
    ...
    {
        public static SceneObject Create(string name, Material material, int latitudeSegments = 32, int longitudeSegments = 32)
        {
            List<float> vertices = [];
            ...
            for (int lat = 0; lat < latitudeSegments; lat++)
            {
                ...
                for (int lon = 0; lon < longitudeSegments; lon++)
                {
                    ...
                    AddVertex(vertices, theta1, phi1);
                    AddVertex(vertices, theta2, phi1);
                    AddVertex(vertices, theta2, phi2);

                    AddVertex(vertices, theta2, phi2);
                    AddVertex(vertices, theta1, phi2);
                    AddVertex(vertices, theta1, phi1);
                }
            }
